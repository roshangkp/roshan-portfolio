...
function typeEffect() {
  ...
  if (!deleting) {
    ...
    charIndex++;
    if (charIndex === currentRole.length) { deleting = true; setTimeout(typeEffect, 1200); return; }
  } else {
    typedText.textContent = currentRole.substring(0, charIndex - 1);
    charIndex--;
...
if (contactForm) {
  contactForm.addEventListener("submit", async function(event) {
    ...
    try {
      ...
      console.error(error);
      formMessage.textContent = "Message could not be sent. Please try again later.";
    } finally {
...
if (typeof QRCode !== "undefined") {
  ...
  if (qrElement) new QRCode(qrElement, {text: window.location.href, width: 128, height: 128});
}
